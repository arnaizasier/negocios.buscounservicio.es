...
if (!$reserva) {
    echo '<p class="text-danger">Reserva no encontrada.</p>';
    exit;
}

// Verificar permisos según el rol del trabajador
if ($reserva['id_negocio'] != $worker_negocio_id) {
    echo '<p class="text-danger">No tienes permiso para ver esta reserva.</p>';
    exit;
}

// Si tiene permisos 1, solo puede ver sus propias reservas
...
if (!$cliente && !empty($reserva['id_cliente_crm'])) {
    ...
    if ($cliente_crm) {
        $reserva['nombre_cliente'] = $cliente_crm['nombre'];
        $reserva['apellidos_cliente'] = $cliente_crm['apellidos'];
        $reserva['email_cliente'] = $cliente_crm['email'];
        $reserva['telefono_cliente'] = $cliente_crm['telefono'];
    }
}
...
<div class="row">
    <div class="col-md-6">
        ...
        <p><strong>Precio:</strong> <?php echo number_format($reserva['precio'], 2); ?> €</p>
    ...
    <div class="col-md-6">
        ...
        <p><strong></strong> <?php echo htmlspecialchars($reserva['telefono_cliente'] ?? 'No disponible'); ?></p>
        <p><strong></strong> <?php echo htmlspecialchars($reserva['email_cliente'] ?? 'No disponible'); ?></p>
