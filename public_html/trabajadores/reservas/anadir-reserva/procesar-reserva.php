...
try {
    if ($tipo_cliente === 'nuevo') {
        ...
        if (!$nombre || !$telefono) {
            ...
            $stmt_nuevo_cliente->execute([$user_id, $negocio_id, $nombre, $apellidos, $telefono, $email]);
        ...
        if ($seleccionado_users_id_input) { 
            ...
            if ($existente_crm) {
                ...
                if ($user_data) {
                    ...
                    $stmt_create_crm_for_user->execute([$user_id, $negocio_id, $final_reserva_id_cliente, $user_data['first_name'], $user_data['last_name'], $user_data['phone'], $user_data['email']]);
                    $id_cliente_crm_db = $pdo6->lastInsertId();
                } else {
