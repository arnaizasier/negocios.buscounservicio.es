...
if (isset($_GET['id_negocio']) && in_array($_GET['id_negocio'], $negocios)) {
    $id_negocio = intval($_GET['id_negocio']);
...
$id_negocio_param_filtro = isset($_GET['id_negocio']) ? '&id_negocio=' . $_GET['id_negocio'] : '';
$id_trabajador_param_filtro = isset($_GET['id_trabajador']) && $_GET['id_trabajador'] !== '' ? '&id_trabajador=' . $_GET['id_trabajador'] : '';
$filtros_params = $id_negocio_param_filtro . $id_trabajador_param_filtro;
        ...
        <div class="main-container">
            <div class="calendario-header mb-4">
                <div class="calendario-header-row">
                    ...
                    <?php
                    if (count($negocios_data) > 1) {
                        echo "<form method='get' id='filtroNegocioForm' class='negocio-filtro-form'>";
                        echo "<label for='negocioSelect' style='margin-right:8px;'>Filtrar por negocio:</label>";
                        echo "<select name='id_negocio' id='negocioSelect' class='form-select' onchange=\"document.getElementById('filtroNegocioForm').submit();\">";
                        foreach ($negocios_data as $n_data) {
                            $n_id = $n_data['negocio_id'];
                            $sel = ($id_negocio == $n_id) ? ' selected' : '';
                            $nombre_negocio = htmlspecialchars($n_data['nombre']);
                            echo "<option value='{$n_id}'{$sel}>{$nombre_negocio}</option>";
                        }
                        if ($id_trabajador !== null) {
                    ...
                    if (!empty($trabajadores_data)) {
                        echo "<form method='get' id='filtroTrabajadorForm' class='trabajador-filtro-form' style='margin-left: 15px;'>";
                        ...
                        echo "<input type='hidden' name='id_negocio' value='{$id_negocio}'>";
                        echo "</select>";
                        echo "</form>";
                    }
                    ?>
                ...
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <?php
                            $hay_filtros = (count($negocios_data) > 1) || !empty($trabajadores_data);
                            ...
                            if ($hay_filtros) {
                                ...
                                if (count($negocios_data) > 1) {
                                    ...
                                    echo "<label for='modalNegocioSelect'><i></i> Filtrar por negocio:</label>";
                                    echo "<select name='id_negocio' id='modalNegocioSelect' class='form-control'>";
                                    foreach ($negocios_data as $n_data) {
                                        $n_id = $n_data['negocio_id'];
                                        $sel = ($id_negocio == $n_id) ? ' selected' : '';
                                        $nombre_negocio = htmlspecialchars($n_data['nombre']);
            ...
            let reservaActualId = null;
                ...
                const calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    datesSet: function(dateInfo) {
                        ...
                        const idNegocioActual = document.getElementById('negocioSelect') ? document.getElementById('negocioSelect').value : '<?php echo $id_negocio; ?>';
                        ...
                        if (idNegocioActual) {
                            currentUrl.searchParams.set('id_negocio', idNegocioActual);
                        }
